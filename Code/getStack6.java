...
// 1. The files are saved with the name "location_X" where X is an integer provided when calling the method.
// 2. The Z stacks are acquired for a single light filter THEN the turret changes filter
// 3. A single .tiff file is produced with all the zstacks in it as opposed to six separate image files. 
public class getStacks6 {
			@SuppressWarnings("deprecation")
			public void getZStacks(File filePath, double zoom_range, double z_increment, 
								  double[] zcorrections, int[] expsr_corrections, int location_number) { 
			...
			//File file = new File(filePath +"/"+"x"+x_position+"_y"+y_position);
			...
			File file = new File(filePath +"/"+"location_"+location_number);
